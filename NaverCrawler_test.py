...
path = "./Recycling_Monitoring/"
search_list = ["삼성 SDS 투자", "포스코 해외 투자"]
...
for search in search_list:
    ...
    if not os.path.exists('./Recycling_Monitoring/%s.csv' %Fname): #기존 파일이 없을 때
        df.to_csv('./Recycling_Monitoring/%s.csv' %Fname, mode='w', index=True, index_label="index", encoding='utf-8-sig') #파일 새로 생성
    else: #기존 파일이 있을 때
        df.to_csv('./Recycling_Monitoring/%s.csv' %Fname, mode='a', index=True, index_label="index", encoding='utf-8-sig', header=False) #파일에 추가하기
