...
search_list = ["LG에너지솔루션 배터리", "삼성SDI 배터리", "SK온 배터리", "롯데에너지머티리얼즈 배터리", "롯데알미늄 배터리", "포스코 배터리", "포스코퓨처엠 배터리", "에코프로비엠 배터리", "SK아이이테크놀로지 배터리", "SKIET 배터리", "에코프로 머티리얼즈 배터리", "에코프로 CNG 배터리", "LG 화학 배터리", "롯데 케미칼 배터리"
]
...
for search in search_list:
    ...
    if search_list.index(search) == 0:
        df.to_csv('./Battery_Monitoring/naver_news_Battery_%s.csv' %today, index=True, index_label="index", mode='w', encoding='utf-8-sig')
    else:
        df.to_csv('./Battery_Monitoring/naver_news_Battery_%s.csv' %today, mode='a', index=True, index_label="index", encoding='utf-8-sig', header=False)
